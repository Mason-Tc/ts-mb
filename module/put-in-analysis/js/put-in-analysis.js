define(function(require, module, exports) {
	...
	var globalVue = new Vue({
		...
		data: {
			...
			warehouseID: '', // 已选仓库ID
			exponentType: 0,
			realNumTotalForSummaryStr: '0件', //入库总数
			realWeightTotalForSummaryStr: '0吨', //入库总重
			monthAverageStr: '0吨', //月均值
			dayAverageStr: '0吨', //日均值
			inStorageSort: 'desc', //入库量
			sortColumn: '',
			basicDatas: {}, // 品名 材质 规格 产地基础数据
			// sortColumns: ["入库量(吨)", "入库量占比", "环比增长率"],
			sortColumns: ["物料描述", "入库数量(件)", "入库量(吨)", "占比"],
			selectedMtxs: {},
		...
		methods: {
			...
			summaryQuery: function(callback) {
				...
				// var apiUrl = app.api_url + '/api/proEnter/list?_t=' + new Date().getTime();
				var apiUrl = app.api_url + '/api/proEnter/enterAnalyze?_t=' + new Date().getTime();
				...
				m.ajax(apiUrl, {
					...
					// 	enterStart: self.filterConditions.beginDate,
					// 	enterEnd: self.filterConditions.endDate
					// },
					data: postData, // 查询参数
			...
			trendQuery: function(callback) {
				...
				var apiUrl = app.api_url + '/api/proEnter/trendAnalyze?_t=' + new Date().getTime();
				m.ajax(apiUrl, {
					data: {
						warehouseIds: self.filterConditions.warehouseId,
						enterStart: self.filterConditions.beginMonth,
						enterEnd: self.filterConditions.endMonth
					},
					dataType: 'json', //服务器返回json格式数据
					type: 'post', //HTTP请求类型
					timeout: 10000, //超时时间设置为10秒；
			...
			materielQuery: function(callback) {
				var self = this;
				var apiUrl = app.api_url + '/api/proEnter/materialTopList?_t=' + new Date().getTime();
				m.ajax(apiUrl, {
					data: {
						warehouseIds: self.filterConditions.warehouseId,
						enterStart: self.filterConditions.beginMonth,
			...
			onItemSliderClick: function($event, index) {
				...
				// if(index === 0) {
				// 	globalVue.getWarehouseList(true);
				// }
			...
			// getBrand: function() { // 品名
			// 	var apiUrl = app.api_url + '/api/sysBusinessBasis/getBrandNameJson?queryFlag=1&brandName=';
			// 	m.ajax(apiUrl, {
			// 		dataType: 'json', //服务器返回json格式数据
			// 		type: 'GET', //HTTP请求类型
			// 		timeout: 10000, //超时时间设置为10秒；
