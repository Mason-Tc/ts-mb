define(function(require, module, exports) {
	...
	m.plusReady(function() {
		...
		aboutVue.getTaskPlatSend(aboutVue.pfList[0].id);
		...
		function detailBack() {
			...
			let target = plus.webview.getWebviewById('outside-list'); 
			target.reload(true);
			backDefault();
		}
		m.back = detailBack;
	...
	var aboutVue = new Vue({
		...
		methods:{
			...
			doSendConfirmComplete:function(detailList){
				swaiting = plus.nativeUI.showWaiting('处理中...');
				m.ajax(app.api_url + '/api/rowcar/outPlatformComplete?_t=' + new Date().getTime(), {
					data:{
			...
			saveOutsidePlatData:function(){
				...
				swaiting = plus.nativeUI.showWaiting('处理中...');
				m.ajax(app.api_url + '/api/rowcar/saveOutsidePlatData?_t=' + new Date().getTime(), {
					data:{
						taskId:aboutVue.proSend.taskId,
						platformId:aboutVue.currentPlatformId,
						jsonData:JSON.stringify(detailList)},
			...
			// 切换月台时，暂时保存数据
			autoSaveOutsidePlatData:function(pfId){
				let detailList=aboutVue.proSend.detailList;
				for(var i=0;i<detailList.length;i++){
					detailList[i].createBy=null;
					detailList[i].updateBy=null;
			...
			// 刷新页面数据
			toRefresh:function(){
				aboutVue.getTaskPlatSend(aboutVue.currentPlatformId);
			},
