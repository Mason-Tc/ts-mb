	...
	var detailVue = new Vue({
		...
		methods: {
			initShow: function() {
				...
				self.processType = self.materialOrCustomerBakItem.auditType;
				if(window.plus) {
				...
				m.ajax(app.api_url + '/api/sys/MaterialOrCustomerBak/detail?_t=' + new Date().getTime(), {
					...
					success: function(data) {
						...
						if(app.debug) {
							console.log("materialBakData:" + JSON.stringify(data));
						}
						if(data.msg) {
							alert(data.msg);
						} else {
							self.dataItem = data.sysMaterialBak;
							self.auditInfo = !self.dataItem ? '' : self.dataItem.auditInfo;
							self.typeTitle = self.dataItem.materialType == '1' ? '品名:' : self.dataItem.materialType == '2' ? '材质:' : self.dataItem.materialType == '3' ? '规格:' : '产地:';
							self.subordinateTitle = self.dataItem.materialType == '1' ? '所属品种:' : self.dataItem.materialType == '2' ? '所属品名:' : '';
							self.subordinateName = self.dataItem.materialType == '1' ? self.dataItem.varietyName : self.dataItem.materialType == '2' ? self.dataItem.brandName : '';
						}
					},
			...
			edit: function(auditStatus) {
				...
				m.ajax(app.api_url + '/api/sys/MaterialOrCustomerBak/submit?_t=' + new Date().getTime(), {
					...
					success: function(data) {
						...
						if(app.debug) {
							console.log("edit materialBakData:" + JSON.stringify(data));
						}
						if(!data.status) {
							alert(data.msg);
						} else {
